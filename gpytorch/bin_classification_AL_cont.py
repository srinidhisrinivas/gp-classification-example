...
def RandSample_cont(gpc, bounds):
    sample = torch.distributions.uniform.Uniform(torch.tensor([bounds[0][0]]), torch.tensor([bounds[0][1]])).sample(sample_shape=(1,2))
    sample = sample.squeeze(0).reshape(1,-1);
    return sample.float()
...
if __name__ == '__main__':
    ...
    y_func = lambda x: torch.sign(torch.sin(x[:, 0] + 2*x[:, 1] * math.pi)).add(1).div(2);
    ...
    ac_funcs = [next_point_ucb, RandSample_cont]
    ...
    for i in range(num_steps):
        ...
        for j, gpc in enumerate(gpc_list):     
            ...
            x_nxt = ac_funcs[j](gpc, [(-1.0,1.0),(-1.0,1.0)]);
            #print(j, x_nxt.shape)
            next_x[j], next_y[j] = x_nxt, y_func(x_nxt);
